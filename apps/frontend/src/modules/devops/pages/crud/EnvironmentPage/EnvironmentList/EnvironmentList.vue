       ...
       <v-data-table
        ...
        >
            ...
            <template v-slot:item.action="{ item }">
                ...
                <edit-button v-if="$store.getters.hasPermission('ENVIRONMENT_UPDATE')"  @click="$emit('update', item)" />
                <delete-button v-if="$store.getters.hasPermission('ENVIRONMENT_DELETE')" @click="$emit('delete', item)" />
            </template>
   ...
   import {DeleteButton, EditButton, ShowButton, SearchInput} from "@dracul/common-frontend"
    ...
    export default {
        ...
        methods:{
            ...
            fetch() {
                this.loading = true
                EnvironmentProvider.paginateEnvironment(
                    this.pageNumber, 
                    this.itemsPerPage,
                    this.search,
                    this.filters,
                    ...
                    this.items = r.data.paginateEnvironment.items
                    this.totalItems = r.data.paginateEnvironment.totalItems
                }).catch(err => {
                    console.error(err)
